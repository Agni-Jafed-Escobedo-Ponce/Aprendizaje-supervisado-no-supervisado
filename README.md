...
El proyecto aplica técnicas de minería de datos siguiendo la metodología del Prof. José Francisco Espinosa Garita. La estrategia se divide en dos fases: primero, se aplica un análisis no supervisado para descubrir patrones ocultos y perfiles de pacientes; posteriormente, estos insights alimentan modelos supervisados para predecir la mortalidad.

📂 Estructura del Repositorio
Aprendizaje-supervisado-no-supervisado/├── README.md                       # Documentación del proyecto├── .gitignore                      # Archivos ignorados (datos pesados, outputs)├── ProyectoApsApns/                # FASE 1: Preparación y Análisis No Supervisado│   ├── scrip1.py                   # Limpieza, transformación e ingeniería de características│   ├── scrip2.py                   # PCA, K-Means y Métricas de Clustering│   ├── scrip2_4_anomalias.py       # Detección de anomalías con Isolation Forest│   └── scrip2_5_asociacion.py      # Reglas de asociación con Apriori└── ProyectoAps/                    # FASE 2: Aprendizaje Supervisado    └── scrip3_supervisado.py       # K-NN, Árbol de Decisión y Red Neuronal (MLP)
🗂️ Descripción Detallada de Archivos
📁 Carpeta: ProyectoApsApns/ (No Supervisado y Preparación)
Esta carpeta contiene los scripts que procesan la base de datos cruda (desde MongoDB/CSV) y ejecutan las 4 categorías de algoritmos no supervisados requeridos.
...
Aplica el algoritmo Apriori para encontrar combinaciones frecuentes de comorbilidades.
...
Para ejecutar estos scripts necesitas tener instalado Python 3.x y las siguientes librerías. Puedes instalarlas todas de una sola vez ejecutando el siguiente comando en tu terminal:
